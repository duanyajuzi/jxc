...
public class DwrDAO extends EntityDAOImpl<BaseModel, Long>
{
	...
	public List<DwrModel> queryDictInfo()
	{
		return getSqlSession().selectList(getMybatisSqlMapNamespace()+".queryDictInfo");
	}
	/**
	 * 描述信息：加载新闻类型
	...
	{
		return getSqlSession().selectList(getMybatisSqlMapNamespace()+".queryDictInfo", dmbm);
	}
	public List<DwrModel> queryGoodsInfo(BaseModel model)
	{
		return getSqlSession().selectList(getMybatisSqlMapNamespace()+".queryGoodsInfo", model);
	}
